...
- **Code and data share the same flat address space.** Code starts at 0x0004 (word 1); address 0x0000 contains a header.
...
| HALT | freeze execution until hard reset |
| RESET | restart: PC = 0x4 (word 1) |
...
## 14. Known Issues (as of April 2026)

Two instructions have a bug in their 64-bit immediate hi32 fetch — a big-endian remnant not yet fixed:

**SETR64** (`register_tasks.vh` line 395): the hi32 word (at PC+8) is selected from the wrong half of the cache doubleword. The condition `r_PC[2]` is inverted. Workaround: avoid SETR64, or use PUSHV64 if the value is going on the stack anyway (but see below).

**PUSHV64** (`stack_tasks.vh` line 143): same problem — hi32 fetch always uses `[63:32]` instead of selecting based on `r_PC[2]`. Workaround: avoid PUSHV64 until fixed.

**LDIDX32** (`alu_extended_tasks.vh` ~line 1096): the 32-bit half-select after a cache fetch has the `addr[2]` condition inverted. Workaround: use LDIDX64 followed by ANDV/ZEXTW to mask to 32 bits.

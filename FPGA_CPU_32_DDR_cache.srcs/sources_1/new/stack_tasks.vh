...
//   PUSH: r_SP -= 4; DDR2[r_SP] = value
//   POP:  value = DDR2[r_SP]; r_SP += 4
...
task t_call_reg;
   begin
      if (r_extra_clock == 0) begin
         r_SP             <= r_SP - 4;
         r_mem_addr       <= r_SP[26:0] - 27'd4;
         r_mem_write_data <= {5'b0, r_PC + 27'd4};  // return after 1-word instruction
         r_mem_write_DV   <= 1'b1;
         r_extra_clock    <= 1'b1;
      end else begin
         if (w_mem_ready) begin
